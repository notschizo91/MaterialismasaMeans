...
/**
 * Make the keyring draggable: getRing() -> {X, Y, R} (centre + outer radius,
 * world mm), onMove({x, y}) fired as the pointer drags on the z=0 plane.
 */
export function setRingDrag(viewer, getRing, onMove) {
  ...
  const overRing = (p) => {
    ...
    const dx = p.x - ring.X;
    const dy = p.y - ring.Y;
    return Math.sqrt(dx * dx + dy * dy) <= ring.R * 1.2;
  };
  ...
  el.addEventListener('pointermove', (ev) => {
    ...
    if (dragging) {
      ...
      el.style.cursor = overRing(p) ? 'grab' : '';
    }
  });
  const stop = (ev) => {
    if (dragging) {
